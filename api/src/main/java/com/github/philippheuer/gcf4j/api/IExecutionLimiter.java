...
public interface IExecutionLimiter {
     ...
     */
    boolean check(IMessageContext ctx);

    /**
     * Executed by the {@link ICommandRegistry} if a command is called and {@link #check(IMessageContext)} returns false.
     *
     * @param ctx The context in which the limiter failed.
     */
    default void onFail(IMessageContext ctx) {
        // NO-OP
    }
}